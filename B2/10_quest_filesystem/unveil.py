class Unveil:
    ...
    def run(self):
        ...
        # Find which directories, being deleted would free up the maximum space, without exceeding 100,000 bytes
        ideal_directories = []
        for _, size in dir_sizes.items():
            if size <= 100_000:
                ideal_directories.append(size)
        
        return max(ideal_directories) if ideal_directories else 0
